...
describe('Account Client Integration', () => {
  it('should return valid AccountWithBalance entity', async () => {
    ...
    const response = await new AccountClient({
      ...
      balanceEndpoint: '/v2/accounts/{accountId}/balance',
      accountEndpoint: '/v2/accounts/',
      baseUrl: 'https://api-sandbox.starlingbank.com/api',
    }).getAccountInformation(accountId);

    expect(response.id).toBe(accountId);
    expect(response.name).toBe(accountName);
    expect(response.defaultCategoryId).toBe(
      '40f0be7a-333b-49f8-9238-b50575b92526'
    );
    expect(response.effectiveBalanceCurrency).toBe(currency);
    expect(response.effectiveBalanceMinorUnits).toBe(balance);
  });
});
